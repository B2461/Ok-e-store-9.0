...
const PremiumIcon: React.FC<{ isActive: boolean }> = ({ isActive }) => (
    <svg 
        ...
        className={`h-8 w-8 transition-all duration-500 ${isActive ? 'text-black' : 'text-orange-500 animate-pulse'}`} 
        viewBox="0 0 24 24" 
...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ cartItemCount }) => {
    ...
    // If active: Yellow Box (activeIconClass)
    // If inactive: Dark circle with Orange border to support floating effect
    ...
    return (
        <nav className="fixed bottom-4 left-0 right-0 max-w-md mx-auto bg-gradient-to-r from-purple-900 via-black to-orange-900 backdrop-blur-md rounded-2xl shadow-[0_0_20px_rgba(249,115,22,0.5)] border border-orange-700/50 z-40">
            <div className="flex justify-around items-center h-14">
                {/* Home */}
                <Link to="/home" className={`relative flex flex-col items-center justify-center gap-1 w-16 h-full p-1 ${(location.pathname === '/home' || location.pathname === '/') ? activeIconClass : inactiveIconClass}`}>
                    <HomeIcon isActive={location.pathname === '/home' || location.pathname === '/'} />
                ...
                {/* Admin */}
                <Link to="/admin" className={`relative flex flex-col items-center justify-center gap-1 w-16 h-full p-1 ${location.pathname.startsWith('/admin') ? activeIconClass : inactiveIconClass}`}>
                    <AdminIcon isActive={location.pathname.startsWith('/admin')} />
                    <span className="text-[10px] font-bold truncate max-w-[60px]">{t('admin')}</span>
                </Link>
